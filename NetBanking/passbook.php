...
$sql = "SELECT pid, pdate, chqno,particular,debit,credit FROM passbook where pid='$id' ";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
     echo "<table class=\" table table-striped table-hover table-bordered\">
     			<tr><th>ID</th><th>Date</th><th>Chq No</th><th>Particular</th><th>Debit amt</th><th>Credit amt</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>" . $row["pid"]. "</td><td>" . $row["pdate"]. "<td>" . $row["chqno"]. "</td>
         <td>" . $row["particular"]. "</td><td>" . $row["debit"]. "</td><td>" . $row["credit"]. "</td>
         </tr>";
     }
     echo "</table>";
} else {
